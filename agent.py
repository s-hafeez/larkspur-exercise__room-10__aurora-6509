...
from support import (MODEL, SYSTEM_PROMPT, call_local, execute_tool, mcp_client,
                     new_session, next_available_day, record_tool_result,  # noqa: F401
                     runtime_preamble)
...
EXTRA_TOOLS: List[Dict[str, Any]] = [    # ✏️ Build 2, step 2.1: schemas for the tools you add
    {
        ...
        "description": (
            "Find the earliest calendar date on which seats are available for a given "
            "origin-destination pair. Use when the customer asks which day they can travel, "
            "how soon they can get out, or what the next available date is — questions about "
            "dates rather than specific flight options. search_alternatives lists flights for "
            "today; this tool answers 'what day' questions. Returns an ISO date string."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "date":   {"type": "string", "description": "YYYY-MM-DD — the disrupted date to search from"},
                "cabin":  {"type": "string", "description": "Fare class code, defaults to Y (economy)"},
            },
...
def tool_list() -> List[Dict[str, Any]]:                   # ✏️ Build 2, step 2.2
    ...
    prints this list."""
    return build_tools() + mcp_client.tools()  # step 2.2: + mcp_client.tools()
    # return build_tools() + EXTRA_TOOLS  # step 2.1: comment out mcp_client.tools() above
...
def build_tools() -> List[Dict[str, Any]]:                 # ✏️ Build 1, step 1.3
    ...
    return [
        ...
        },
        {
            ...
            "description": "Search for available rebooking options on a disrupted booking. Call after get_flight_status confirms a cancellation or significant delay. Returns a list of alternatives, each with an option_id that hold_seat needs.",
            "input_schema": {
                "type": "object",
